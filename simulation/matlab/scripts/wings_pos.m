...
X_des = 5;  % Start at origin
Y_des = 5;  
Z_des = -10; % Start with small altitude

% PID Coefficients for position control
Kp_pos_xy = 5;  Ki_pos_xy = 0; Kd_pos_xy = 2000;  % Fixed position gains
Kp_Z = 500;       Ki_Z = 0;          Kd_Z = 5000;      % Z position control gains

% PID Coefficients for angle control
Kp_ang = 0.006; Ki_ang = 0; Kd_ang = 0;
...
while i < (timePeriod * frequency)
    ...
    alpha = 30*(1/(1 + exp(-4*(current_time - 30))));
    
    if current_time > 29
        Kp_Z = 700;    Ki_Z = 0;   Kd_Z = 2000;
        % Kp_pos_xy = 4.259; Kd_pos_xy = 2967; 
        % Kp_ang = 0.0770; Kd_ang = 0.2003;
    end
    ...
    motor_phi_adjustment = phi_ctrl * 1;   % Scale factor for roll control
    motor_theta_adjustment = theta_ctrl * 1; % Scale factor for pitch control
    ...
    fade_factor = (1 - 1/(1 + exp(-4*(current_time - 60))));  % Fixed timing like Z-only
    stINIT(13) = w1;
...
%     sg = @(x) 1/(1 + exp(1)^(-x));
    ...
    alpha_R = 4; % linear damping co-eff of drag
