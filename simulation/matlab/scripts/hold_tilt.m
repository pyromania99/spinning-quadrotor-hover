...
tspan = [0 1/frequency];
stINIT = [ 0, 0 ,0 ,0 ,0.2 ,0 ,0 ,0 ,0 ,0 ,0 ,0 , w_init, w_init, w_init, w_init];
...
Z_des = 0; % Desired altitude
phi_world_des = 0;    % Desired world frame roll (rad)
theta_world_des = 0.2; % Desired world frame pitch (rad) 

% PID Coefficients - MERGED ATTITUDE GAINS
Kp_Z = 500;       Ki_Z = 0;          Kd_Z = 5000;      % Z position control gains
Kp_att = 10;     Ki_att = 0;       Kd_att = 0;      % Attitude control gains (phi & theta)
...
while i < (timePeriod * frequency)
    ...
    force_scale = 10; % Scale factor to convert control to force
...
if ~isempty(all_states) && size(all_states, 1) > 0
    ...
    plot([0 all_times(end)], [rad2deg(phi_world_des) rad2deg(phi_world_des)], 'r--', 'LineWidth', 1.5); hold off;
...
if ~isempty(all_states) && size(all_states, 1) > 0
    ...
    plot([0 all_times(end)], [rad2deg(theta_world_des) rad2deg(theta_world_des)], 'r--', 'LineWidth', 1.5); hold off;
...
% Add debugging plot for motor differences
figure(9);
if size(motor_speed, 2) > 0
    plot(time_des_arr, motor_speed(1,:) - mean(motor_speed,1), 'r-', 'DisplayName', 'w1 - avg'); hold on;
    plot(time_des_arr, motor_speed(2,:) - mean(motor_speed,1), 'g-', 'DisplayName', 'w2 - avg');
    plot(time_des_arr, motor_speed(3,:) - mean(motor_speed,1), 'b-', 'DisplayName', 'w3 - avg');
    plot(time_des_arr, motor_speed(4,:) - mean(motor_speed,1), 'k-', 'DisplayName', 'w4 - avg');
    hold off;
