...
tspan = [0 1/frequency];
stINIT = [ 0, 0 ,0 ,0.2 ,0 ,0 ,0 ,0 ,0 ,0 ,0 ,0 , w_init, w_init, w_init, w_init];
%stIN = [x, y, z, phi, theta, psi, u, v, w, p, q, r, w1, w2, w3, w4];
...
phi_world_des = 0.01;    % Desired world frame roll (rad)
theta_world_des = 0; % Desired world frame pitch (rad)

% PID Coefficients - MERGED ATTITUDE GAINS WITH ADAPTIVE VALUES
Kp_Z = 500;           Ki_Z = 0;          Kd_Z = 5000;      % Z position control gains (fixed)
Kp_att_max = 500;     Ki_att = 0;       Kd_att = 0;      % Attitude control gains (phi & theta)
Kp_att_min = 50;      % Minimum attitude gain near target
Kff_att = 0;          % Feedforward gain for attitude control

% Adaptive gain parameters
att_deadband = deg2rad(0.5);  % Angle within which to start reducing attitude gains (rad)
...
while i < (timePeriod * frequency)
    ...
    psi_rate_filtered = 0.7 * psi_rate + 0.3 * psi_rate_prev;  % Simple filter
    ...
    psi_predicted = psi_current + psi_rate_filtered * dt/100 + offset;
    ...
    % Adaptive attitude gain - reduces as we get closer to target orientation
    if att_error_combined <= att_deadband
        att_gain_factor = att_error_combined / att_deadband;  % 0 to 1 scale
...
if ~isempty(all_states) && size(all_states, 1) > 0
    ...
    % --- Adaptive window size from times ---
    targetWindowSec = 0.1; % seconds
    if length(all_times) >= 2
        median_dt = median(diff(all_times));
        windowSize = max(5, 2*round(targetWindowSec/median_dt)+1); % odd for sgolay
    else
        median_dt = 0.01; % fallback dt
        windowSize = 5;
    end

    % --- Smooth world-frame velocities ---
    x_rate_world_avg = sgolayfilt(x_rate_world, 3, min(windowSize, length(x_rate_world))); 
    y_rate_world_avg = sgolayfilt(y_rate_world, 3, min(windowSize, length(y_rate_world))); 

    % --- Differentiate to get accelerations ---
    a_x_world = gradient(x_rate_world_avg, all_times);
    a_y_world = gradient(y_rate_world_avg, all_times);

    % --- Low-pass filter accelerations ---
    fs = 1/median_dt; % sample rate [Hz]
    fc = 0.5;  % cutoff frequency [Hz]
    [b,a] = butter(1, fc/(fs/2), 'low');

    a_x_world_filt = filtfilt(b,a,a_x_world);
    a_y_world_filt = filtfilt(b,a,a_y_world);

    % --- Compute z_rate_world properly using R_body_to_world ---
    z_rate_world = zeros(size(all_times));
    for j = 1:length(all_times)
        psi = all_states(j,6);
        phi = all_states(j,4);
        theta = all_states(j,5);
        ...
        world_vel = R_body_to_world * [u; v; w];
        z_rate_world(j) = world_vel(3); % vertical velocity in world frame
    end

    % --- Differentiate to get world vertical acceleration ---
    z_rate_world_avg = sgolayfilt(z_rate_world, 3, min(windowSize,length(z_rate_world)));
    a_z_world = gradient(z_rate_world_avg, all_times);
    a_z_world_filt = filtfilt(b,a,a_z_world);

    % --- Estimate thrust vector magnitude ---
    thrust_total = sqrt(a_x_world_filt.^2 + a_y_world_filt.^2 + (a_z_world_filt+g).^2);

    % --- Expected roll & pitch ---
    phi_exp   = atan2(a_y_world_filt, thrust_total);
    theta_exp = atan2(-a_x_world_filt, sqrt(a_y_world_filt.^2 + (a_z_world_filt+g).^2));

    % --- Smooth outputs ---
    polyOrder = 3;
    if windowSize > polyOrder
        phi_exp   = sgolayfilt(phi_exp, polyOrder, min(windowSize,length(phi_exp)));
        theta_exp = sgolayfilt(theta_exp, polyOrder, min(windowSize,length(theta_exp)));
    end

    % --- Plot ---
    subplot(2,1,1);
    plot(all_times, rad2deg(phi_exp), 'b-', 'LineWidth', 1.5); hold on;
    plot([0 all_times(end)], [rad2deg(phi_world_des) rad2deg(phi_world_des)], 'r--', 'LineWidth', 1.5);
    hold off;
    title('Roll: expected (from thrust-normalized a) vs world attitude (\phi)');
    ylabel('deg');
    legend('expected (a)', 'world \phi', 'Location', 'best');
    grid on;

    subplot(2,1,2);
    plot(all_times, rad2deg(theta_exp), 'r-', 'LineWidth', 1.5); hold on;
    plot([0 all_times(end)], [rad2deg(theta_world_des) rad2deg(theta_world_des)], 'r--', 'LineWidth', 1.5);
    hold off;
    title('Pitch: expected (from thrust-normalized a) vs world attitude (\theta)');
    ylabel('deg');
    xlabel('Time (s)');
    legend('expected (a)', 'world \theta', 'Location', 'best');
    grid on;
else
    subplot(2,1,1); plot([0 1],[0 0],'b-'); title('Expected Roll - No Data'); 
    subplot(2,1,2); plot([0 1],[0 0],'r-'); title('Expected Pitch - No Data'); 
end
